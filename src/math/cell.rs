...
mod tests {
  ...
  use super::{Attach, Cell, OpenSet, Skeleton, MathError};
  ...
  fn test_fetch_cell_by_point() {
    ...
    assert!(matches!(sk.fetch_cell_by_point(TestPoint("Z")), Err(MathError::NoPointFound)));
  }
  ...
  fn test_incident_and_filter_errors() {
    ...
    assert!(matches!(sk.incident_cells(0, 1), Err(MathError::DimensionMismatch)));
    assert!(matches!(sk.filter_incident_by_dim(0, 1), Err(MathError::DimensionMismatch)));

    // Bad index within valid dim should be InvalidCellIdx
    assert!(matches!(sk.incident_cells(99, 0), Err(MathError::InvalidCellIdx)));
  }
}
